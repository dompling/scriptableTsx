...
var Widget = class extends Base_default {
  constructor() {
    ...
    this.getCovid19 = async (params) => {
      ...
      if (response.ret === 0 && response.data && response.data instanceof Array) {
        const covid_19 = response.data.find((item) => item.date === this.today);
        if (covid_19)
          return covid_19;
      }
    };
    ...
    this.render = async () => {
      ...
      return /* @__PURE__ */ h("wbox", {
        ...
        textColor: "#005dff",
        font: addumFont
      }, this.city?.city || "未找到定位")), /* @__PURE__ */ h("wspacer", {
        length: 5
      }), /* @__PURE__ */ h("wstack", {
        verticalAlign: "center"
        ...
        return /* @__PURE__ */ h(Fragment, null, /* @__PURE__ */ h("wstack", {
          ...
          height: 30,
          borderRadius: 4
        })), (this.news && this.news?.length - 1) !== index && /* @__PURE__ */ h("wspacer", null));
  ...
  async getCovid19News() {
    ...
    let {city = ""} = this.location.postalAddress;
    city = city.replace("省", "").replace("市", "");
    const py = pinyin_default.firstChar(city);
